...
public class bill_report extends JFrame implements ActionListener
{
    Container cp;
    PrinterJob pj;
    PageFormat pf;
    ...
    public void connect()
    {
        ...
        Class.forName("oracle.jdbc.driver.OracleDriver");
         cn=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","bca","bca");
		st=cn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
		rs=st.executeQuery("select * from bill_detail");
		}catch(Exception e) { }
    ...
    public void grand_total()
    {
        try
        {
            String query1="Select sum(bno) from bill_detail where bno=? group by bno";
    ...
    {
       try{
        String query = "SELECT * FROM bill_detail " + wCOND;
        st = cn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
    ...
    {
        String query = "SELECT bno FROM bill_detail " ;
        try{
    ...
    public bill_report()
   {
     ...
     lblempid = new JLabel("Bill Id :  ",SwingConstants.RIGHT);
    ...
    public void actionPerformed(ActionEvent e)
   {
      ...
      if(cmd.equals("Show"))
      {
        wCOND = " WHERE bno='" + cmbemp_id.getSelectedItem()+"'";
        showGridData();
      }
    ...
    void showGridData()
  {
    openRecord();
    if(rc==0)
      JOptionPane.showMessageDialog(null,"Data not Found.","Bill Report",JOptionPane.INFORMATION_MESSAGE);
    else
    {
      ...
      remove(t);
      t=new JPanel();
      t.setBorder(BorderFactory.createTitledBorder("Bill_report"));
  ...
  {
    bill_report dgr=new bill_report();
    dgr.show();
  }
